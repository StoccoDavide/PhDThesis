...
\begin{itemize}
  \setlength\itemsep{0em}
  \item The \emph{length} of an expression, in terms of the number of characters used to internally represent the expression. A possible measure of that exploit the length of an expression is the following
  %
  \begin{equation*}
    N_\beta = \begin{cases}
...
\begin{definition}[Signature of a Hierarchical Representation~\cite{zhou2007symbolic}]
  ...
  \begin{itemize}
    \setlength\itemsep{0em}
    \item We define$ s(v_i) = f_i(\delta_{i,j}, \dots, \delta_{i,k_i}) ~ \text{mod} ~ p$.
...
\begin{definition}[Full-Pivoting \ac{LU} Decomposition]
  Given a matrix $\m{A} \in \mathbb{R}^{m \times m}$, with $m \geq n$, the full-pivoting \ac{LU} decomposition is defined as the process of decomposing $\m{A}$ into the product of
  %
  \begin{itemize}
...
\begin{definition}[Full-Pivoting \ac{FFLU} Decomposition]
  Given a matrix $\m{A} \in \mathbb{R}^{m \times m}$, with $m \geq n$, the full-pivoting \ac{FFLU} decomposition is defined as the process of decomposing $\m{A}$ into the product of
  %
  \begin{itemize}
    \setlength{\itemsep}{0.0em}
...
\begin{example}[Fill-in reduction in \ac{LU} factorization]
  ...
  \begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
      ...
      \caption{Original sparsity pattern of non-pivoted $\m{L}$ and $\m{U}$ factors (15918 non-zero elements).}
    \end{subfigure} \\[1.0em]
    \begin{subfigure}[t]{0.45\textwidth}
      \centering
