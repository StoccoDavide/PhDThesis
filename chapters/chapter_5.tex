...
In the previous chapter, we presented a methodology for the automatic index reduction of \ac{DAE} systems. The index reduction algorithm is based on the separation of the system into differential and algebraic parts with the help of symbolic linear algebra, \ie{}, \ac{LU}, or \ac{FFLU} matrix factorization. No information on the \ac{DAE} system structure is leveraged during the reduction process. For this reason, this methodology can be applied to generic \acp{DAE} linear in the state derivatives. Numerical integration goodness is showcased by assessing the order of the numerical integration scheme and the accuracy of the projection. In this chapter, we take for granted the correctness of the proposed methodology and we focus on the application and capabilities of the index reduction algorithm. Hence, the algorithm is applied to a variety of \ac{DAE} systems, which are chosen to demonstrate the capabilities, the potential, as well as the wide range of applicability of the proposed methodology. The examples are divided into three main categories: \emph{multibody dynamics}, \emph{trajectory prescribed path control}, and \emph{electrical circuits}. Each of these categories is characterized by a different structure of the \ac{DAE} system, which is analyzed in detail and helps us to understand the effectiveness of the proposed methodology. The examples are mainly taken from the testsets~\cite{lioen1998test, mazzia2003test, mazzia2008test, mazzia2012test} and are the following.
%
\begin{enumerate}
  ...
  \end{enumerate}
  \item Electrical circuits field:
  \begin{enumerate}
    \setlength\itemsep{0.0em}
...
\begin{figure}
  \centering
  \includegraphics[width=9cm]{figures/chapter_5/car_axis}
...
\begin{equation*}
  \begin{cases}
    ...
    \bm{\Phi}(\m{p}) = \m{0}
  \end{cases} \, \text{,}
  \quad \text{with} \quad \jac{\bm{\Phi}}{\m{p},\m{q}}(\m{p}, \m{q}, t) = \dfrac{\partial\bm{\Phi}(\m{p}, \m{q}, t)}{\partial(\m{p}, \m{q})}
  \, \text{,}
\end{equation*}
%
...
\begin{equation*}
  \m{M}_e(\m{p}, \m{q}) = \begin{bmatrix}
    0 & \rho \ell_1(\cos(\phi_{12})\m{c}_1^\top + \sin(\phi_{12})\m{c}_2^\top) \m{q} & 0 \\
    \rho \ell_1(\cos(\phi_{12})\m{c}_1^\top + \sin(\phi_{12})\m{c}_2^\top) \m{q} & \m{q}^\top\m{M}_{d}\m{q} + 2\rho\m{c}_{12}^\top\m{q} & 0 \\
    0 & 0 & 0
  \end{bmatrix}
\end{equation*}
%
...
\end{equation*}
%
and $phi_{12} = \phi_1 - \phi_2$. For the force term $\m{f}_e(\m{p}, \m{q}, \m{p}^\prime, \m{q}^\prime)$ the expression is
%
\begin{equation*}
  \m{f}_e(\m{p}, \m{q}, \m{p}^\prime, \m{q}^\prime) = \begin{bmatrix}
      \rho \ell_1 \phi_2^{\prime2}(\cos(\phi_{12})\m{c}_2^\top - \sin(\phi_{12})\m{c}_1^\top) \m{q}-2 \rho \ell_1 \phi_2^\prime(\cos(\phi_{12})\m{c}_1^\top + \sin(\phi_{12})\m{c}_2^\top) \m{q}^\prime \\[0.5em]
      \rho \ell_1 \phi_1^{\prime2}(\sin(\phi_{12})\m{c}_1^\top - \cos(\phi_{12})\m{c}_2^\top) \m{q}-2 \rho \phi_2^\prime \m{c}_{12}^\top \m{q}^\prime-2 \phi_2^\prime {\m{q}^\prime}^\top \m{M}_{d} \m{q} \dots\\
...
%
\begin{align*}
  f_{d}(\m{p}, \m{q}, \m{p}^\prime, \m{q}^\prime) &= \phi_2^{\prime2} \m{M}_{d} \m{q}+\rho(\phi_2^{\prime2} \m{c}_{12}+\ell_1 \phi_1^{\prime2}(\cos(\phi_{12}) \m{c}_1 \dots \\
  &+ \sin(\phi_{12}) \m{c}_2)+2 \phi_2^\prime \m{B} \m{q}^\prime)-\rho \gamma\left(\sin \phi_2 \m{c}_1+\cos(\phi_2) \m{c}_2\right) \, \text{.}
\end{align*}
