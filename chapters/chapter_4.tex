  ...
  \begin{algorithmic}[1]
    ...
    \Procedure{ReduceIndex}{$\mFv$} \Comment{Index reduction procedure}
      ...
      \While{$\mAv$ is singular}
        \State $\displaystyle\triangleright$ Differential and algebraic equations separation (Section~\ref{chap4:sec:separation})
        \State $\mLv, \, \mUv, \, \mP, \, \mQ \gets \mathrm{MatrixFactorization}(\mAv)$ \Comment{Factorization of $\mAv$}
        \State $\m{v}_d(\mx, \m{v}, \, t) \gets \mathrm{NewVeilings}(\mAv, \, \mv)$ \Comment{New veils from factorization of $\mAv$}
