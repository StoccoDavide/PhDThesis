...
However, as we mentioned before, there exist multiple definitions of the index of a \ac{DAE} system, and each of these definitions is specifically tailored to a particular index reduction technique. As a consequence, there are various approaches to index reduction, whose characteristics also depend on factors like the capabilities of the available package, the user's expertise, and the specific problem at hand. Each method has its own pros and cons, being more or less suitable for different types of \acp{DAE} and varying in computational costs, implementation requirements, and theoretical foundations.
 ...
 %and the symbolic computation techniques, which include the ``standard'' differential index reduction method as well as the built-in \Maple{} functions for index reduction. These latter methods are particularly effective for small-scale complicated \acp{DAE}, especially if \ac{SA} methods are not applicable or faulty.

%The most prevalent index reduction methods include the Pantelides algorithm, falling under the umbrella of structural analysis methods, and symbolic computation techniques, encompassing the "standard" differential index reduction method along with built-in Maple functions for index reduction. Symbolic computation methods are particularly effective for small-scale complex \acp{DAE}, especially when structural analysis methods are inapplicable or problematic.
